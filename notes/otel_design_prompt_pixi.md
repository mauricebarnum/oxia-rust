# Prompt: OpenTelemetry Instrumentation Design Document for a Rust Library (msb)
...
> - **Before beginning any analysis, verify that all four inputs below are resolved (not placeholder values). If any input is missing or still a placeholder, halt immediately and request the missing information. Do not proceed with partial inputs and do not infer or hallucinate repository structure.**
>
> **Inputs:**
>
> - Rust library to analyze: `client`
> - Existing Go library that serves as the behavioral reference: `ext/oxia`
> - Target OpenTelemetry Semantic Conventions Version: `v1.41.0`
> - Additional repository docs/tests/examples to inspect: `client/cli`
> - If it exists, incorporate the contents of the file `otel-instrumentation-design.md` from a previous iteration or another agent
>
> **Objective:**
>
...
> - The library must only use the chosen framework's _API/facade layer_ in-library. It must not configure exporters, providers, SDK pipelines, or application-level runtime setups internally.
...
>   - _Bounded_ — document the bound and its source.
>   - _Potentially unbounded_ — flag it as a risk requiring explicit reviewer sign-off before implementation proceeds.
...
> | ------------------------- | ------------------ | ---------------------------- | --------------------------- | -------------------------- |
> | _e.g., `Client::connect`_ | `"client.connect"` | `SpanKind::Client`           | `"client.connect.duration"` | Warm path                  |
>
