...
async fn main() -> Result<()> {
    ...
    let config = crate::config::Builder::new().service_addr(server_address).build()?;
    let mut client = Client::new(config);
    client.connect().await?;

    // let key = "k".to_string();
    // let val = Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string().as_bytes().to_vec();
    //
    // println!("{:?}", client.get(key.clone()).await?);
    scan_dump(&client).await?;

    // client.put(key.clone(), val).await?;
    // println!("{:?}", client.get(key.clone()).await?);
